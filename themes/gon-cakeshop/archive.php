				...
				<div class="page-content blog-page <?php echo esc_attr($blogclass); if($blogsidebar=='left') {echo ' left-sidebar'; } if($blogsidebar=='right') {echo ' right-sidebar'; } ?>">
					<header class="archive-header">
						<h1 class="archive-title">
							<?php
							if (is_day()) :
								printf(esc_html__('Daily Archives: %s', 'gon-cakeshop'), '<span>' . get_the_date() . '</span>');
							elseif (is_month()) :
								printf(esc_html__('Monthly Archives: %s', 'gon-cakeshop'), '<span>' . get_the_date(_x('F Y', 'monthly archives date format', 'gon-cakeshop')) . '</span>');
							elseif (is_year()) :
								printf(esc_html__('Yearly Archives: %s', 'gon-cakeshop'), '<span>' . get_the_date(_x('Y', 'yearly archives date format', 'gon-cakeshop')) . '</span>');
							else :
								esc_html_e('Archives', 'gon-cakeshop');
							endif;
							?>	
						</h1>
					</header>
