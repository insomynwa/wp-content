      ...
      function(response){
        ...
        if( result.status ) {
          $('#addr'+i).html("<td><input type='hidden' value='"+ result.data.batch.id+"' name='batch_id[]'>"+ (i)+"</td>"+
            ...
            "<td>"+
              "<a id='' class='delete-batch-row btn btn-default'><span id=''></span>Delete</a>"+
              "<a id='' data-product-id='"+ result.data.product.id+"' data-batch-id='"+ result.data.batch.id+"' class='update-batch-btn btn btn-default'>Update</a>"+
            "</td>");
