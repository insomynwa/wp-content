...
<?php do_action( 'dokan_product_listing_filter_before_form' ); ?>
    ...
    <form method="get" class="dokan-form-inline dokan-w6 dokan-product-search-form">
        ...
        <button type="submit" name="product_listing_search" value="ok" class="dokan-btn dokan-btn-theme"><?php _e( 'Cari', 'dokan-lite'); ?></button>

        <?php wp_nonce_field( 'dokan_product_search', 'dokan_product_search_nonce' ); ?>

        <div class="dokan-form-group">
            <input type="text" class="dokan-form-control" name="product_search_name" placeholder="<?php _e( 'Cari produk', 'dokan-lite' ) ?>" value="<?php echo isset( $_GET['product_search_name'] ) ? $_GET['product_search_name'] : '' ?>">
        </div>
