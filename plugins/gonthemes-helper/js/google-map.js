...
function gon_loadMap($) {
    $('.google-map-canvas').each(function () {
        var $$ = $(this);
        // We use the gon_geocoder
        var gon_geocoder = new google.maps.Geocoder();
        gon_geocoder.geocode({'address': $$.data('address')}, function (results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                ...
                if ( markerPositions && markerPositions.length ) {
                    markerPositions.forEach(
                        function(mrkr) {
                            gon_geocoder.geocode( { 'address': mrkr.place }, function (res, status) {
                                if ( status == google.maps.GeocoderStatus.OK ) {
                                    new google.maps.Marker({
...
function gon_loadApi($) {
    var apiKey = $('.google-map-canvas').data('google-map-api');
